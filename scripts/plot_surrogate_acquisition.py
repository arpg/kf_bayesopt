...
#plt.rcParams['figure.figsize'] = 5, 10 #this can change figure window size
import numpy as np
...
def plot_x(ros_data):
    ...
    #print('whole length of data is ', len(ros_data.data))
    lenSampleData = len(_sampleData)
    #print('lenSampleData is ', lenSampleData)
    _sampleX = []
    _sampleY = []
    if ros_data.layout.dim[0].size == 1 : #one dimension, length must be 2* numbers of sample
        ...
        if ros_data.layout.dim[2].size == 1:
                rospy.loginfo(rospy.get_caller_id() + 'stay active')
                plt.pause(9999999)
    else :
        #print(len(_sgAcDataSg))
        #print(len(_sgAcDataAc))
        #print(xlen)
        _sampleX    = _sampleData[0:(2*lenSampleData/3)] #2 dimension + 1 dimension z (cost), lenSampleData should be multiple of three, length must be 3* numbers of sample
        ...
        scatter1 = ax.scatter(samplex,sampley,_samplez, c = 'r',s = 40)
        ax.set_xlabel('noise1')
        ax.set_ylabel('noise2')
        ax.set_zlabel('Cost')
        wire2 = bx.plot_wireframe(cooX,cooY,cc)
